...
class PairwiseExperimentRunner:
    """
    Runs a pairwise-comparison experiment using:
    - a prepared dataloader
    - a logger
    ...
    def _validate(self) -> None:
        ...
        if loader_items != comparator_items:
            raise ValueError("Comparator items do not match dataloader items in order/content.")

        if getattr(self.comparator, "logger", None) is not self.logger:
            raise ValueError("Comparator.logger must be the same logger passed to the runner.")
    ...
    def run(self):
        ...
        result = self.comparator.compare_items(self.dataloader.tie_sheet)

        self.logger.log_run_end(
            run_id=self.run_id,
            extra={
                "num_logged_comparisons": int(getattr(self.comparator, "num_comparisons", 0)),
            },
        )
        self.logger.close()
