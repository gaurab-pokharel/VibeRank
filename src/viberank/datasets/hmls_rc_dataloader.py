...
class RankCentralityConfig:
    ...
    @classmethod
    def from_yaml(cls, config_path: str | Path) -> "RankCentralityConfig":
...
class RankCentralityDataLoader:
    """
    Data loader for Rank Centrality pairwise-comparison experiments.

    Same household loading and JSON prep as HMISPairwiseDataLoader,
    but the tie sheet is always C(n, 2) unordered pairs.
    """

    def __init__(self, config: RankCentralityConfig):
        self.config = config
    ...
    @classmethod
    def from_yaml(cls, config_path: str | Path) -> "RankCentralityDataLoader":
        return cls(RankCentralityConfig.from_yaml(config_path))
    ...
    def get_run_metadata(self) -> dict[str, Any]:
        return {
            ...
            "selected_households_path": str(
                self.config.selected_households_path
            ),
        }